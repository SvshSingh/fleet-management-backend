...
hundred robots at 1 Hz is 500 msg/s — roughly a 300× increase.
...
4. **Then** move state to Redis so the backend is stateless and horizontally
   ...
   the design genuinely changes rather than being tuned — and note that the
...
₹-per-robot-per-month wasteful at 500 robots on LTE — most of it re-transmitting facts
...
1. **Do not send what has not changed.** A robot that is `idle` and stationary
   ...
   robots move in only 307 of 1,448 samples: **79% of samples are a robot repeating
   itself.** That single change is roughly a 4× reduction, and I would do it first
...
2. The backend's watchdog ages the robot `live → stale → lost`, so the dashboard shows
   uncertainty rather than a frozen position presented as current.
...
`?since=<version>` replays exactly what it missed from `Hub.replay_since` — and if its
...
zero, and a client reconnecting with `?since=4000` gets `None` and re-syncs — correct,
